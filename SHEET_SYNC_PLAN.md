...
| U2 | **Brandmark bolt red in dark mode** | ✅ done + DEPLOYED (2026-06-03) | The spinning GMP brandmark is a GIF (black text + red bolt). Light mode already shows the red bolt. In dark mode the GIF is flattened to a white silhouette (so the text stays visible) — which also whitened the bolt. A first attempt **rebuilt the whole mark as an inline SVG**, but that drifted the font + bolt shape (the art is a specific typeface/bolt drawing), and the user pushed back ("I asked for one color change"), so it was **reverted**. **Final approach:** keep the original GIF 100% untouched and, in dark mode only, overlay the *original* bolt — cut from the same artwork (`web/public/assets/gmp-bolt-red.png`, ~4KB, extracted via a radial-masked red-pixel crop) — back in red, sized/positioned as fractions of the square mark so it aligns to the GIF's static centered bolt and scales responsively. The `.hero-mark img` rules are scoped with `:not(.hero-bolt-overlay)` so the white-flatten filter doesn't touch the overlay. Verified both themes via headless render. **Lesson: make the minimal change the user asked for; don't redraw branded assets.** |
| U3 | **Agency client-picker readability** | ✅ done + DEPLOYED (2026-06-03) | Root cause was **not** spacing taste — it was a **Tailwind v4 cascade-layer bug**: `strategy.css` + `sections-v6.css` were `@import`ed **unlayered**, so strategy.css's `* { margin:0; padding:0 }` reset beat every Tailwind margin/padding utility (per CSS layers, *unlayered > layered* regardless of specificity). That collapsed `px-/py-/p-/m-/gap` spacing across **all** React shell views (AgencyHome, ClientView, Reporting, SignIn, Admin) — only `gap`/borders/colors survived. **Fix:** import both legacy sheets into `layer(base)` in `web/src/index.css` so Tailwind's `utilities` layer wins; the strategy dashboard sections use *zero* Tailwind utilities (verified), so no regression. Also fixed `font-[var(--font-display)]` → `font-[family-name:var(--font-display)]` (the former compiled to `font-weight`, a no-op, leaving card titles in the sans body font). Verified with a headless render of the exact DOM + live CSS. The "still looks like crap" reports were a **stale cache** (pre-redesign build) — see U8. |
| U4 | Optimize 12MB hero GIF | not started | `web/public/assets/gmp-brandmark.gif` is 12MB (in git history) — convert to optimized GIF/WebM/video; heavy on every page load. *(An SVG rebuild that would have eliminated it was reverted to preserve the exact original font/bolt — see U2. A faithful optimization must re-encode the same artwork, not redraw it.)* |
...
| U8 | **Stale-deploy cache** | ✅ done + DEPLOYED (2026-06-03) | Root cause of the *entire session's* "still seeing the old version after a hard refresh." Firebase Hosting served `index.html` with the default `cache-control: max-age=3600`, so browsers kept loading stale HTML → stale hashed bundles for up to an hour post-deploy. **Fix** (`firebase.json` `hosting.headers`): `**` → `no-cache, no-store, must-revalidate` (HTML always revalidates → deploys go live instantly), `/assets/**` → `public, max-age=31536000, immutable` (content-hashed assets cached forever). **Gotcha:** Firebase header precedence is **last-matching-wins** for a duplicate key — the catch-all `**` must be listed *before* the specific `/assets/**` rule, or `**` clobbers it. Verified live: HTML=no-cache, assets=immutable. |
