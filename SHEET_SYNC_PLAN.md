...
| U2 | **Brandmark → inline SVG (red bolt both modes)** | ✅ done + DEPLOYED (2026-06-03) | The spinning GMP brandmark was a 12MB GIF (dark artwork). First pass flattened it white in dark mode via a CSS `filter`, but that also whitened the red bolt. Since `filter` can't recolor *part* of a raster, **rebuilt the brandmark as an inline SVG** in `Hero.tsx`: a `<textPath>` ring ("GROWTH · MARKETING · PRO ·") colored `var(--ink-0)` (dark-on-light, cream-on-dark automatically) that spins 30s/clockwise (matching the GIF), and a static lightning bolt stroked `var(--red)` so it stays **red in both light and dark mode** (the user's request). Respects `prefers-reduced-motion`. Verified via headless render of both themes against the live CSS. |
...
| U4 | Optimize 12MB hero GIF | ✅ done + DEPLOYED (2026-06-03) | Resolved by U2: the 12MB `gmp-brandmark.gif` was deleted (`git rm`) and replaced by a ~1KB inline SVG. `dist/assets` dropped from ~14MB → 1.8MB. *(The blob still lives in git history; a history rewrite to reclaim repo size is a separate, optional cleanup.)* |
