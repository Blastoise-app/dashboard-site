import { cell, parseSv, parseCpc, normalizeStatus } from './util.mjs';

const SEO_LEVER_COUNT = 3; // first 3 columns after KEYWORD INFO belong to "SEO (ON-SITE)"

export function parseGeoTracker(rows) {
  ...
  const levers = leverLabels.map((l, idx) => ({
    id: toCamel(l.label),
    label: l.label,
    col: l.col,
  ...
  return {
    levers: levers.map(({ col, ...rest }) => rest),
    keywords
  };
}
